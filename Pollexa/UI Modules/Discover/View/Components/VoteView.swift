...
final class VoteView: UIView {
    ...
    private var option: Post.Option?
    
    /// A boolean indicating if the current user has voted for this option.
    private var isVotedByCurrentUser: Bool = false {
        didSet {
    ...
    private var isVoted: Bool = false {
        didSet {
            updateVoteViewAppearance()
        }
    }
    ...
    private var voteRatio: Double = 0.0 {
        didSet {
            updateRatioLabel()
        }
    }
...
private extension VoteView {
    ...
    final func updateVoteViewAppearance() {
